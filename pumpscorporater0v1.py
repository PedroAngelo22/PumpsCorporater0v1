...
# --- ALTERAÇÃO 1: Atualizar a função do diagrama para usar o nome customizado ---
def gerar_diagrama_rede(sistema, vazao_total, distribuicao_vazao, fluido, materiais_combinados, fluidos_combinados):
    ...
    for i, trecho in enumerate(sistema['antes']):
        ...
        # Usa o nome do trecho (trecho['nome']) no label
...
# --- ALTERAÇÃO 2: Adicionar o campo de nome na interface do trecho ---
def render_trecho_ui(trecho, prefixo, lista_trechos, materiais_combinados):
    # Adiciona um campo de texto para o nome do trecho
...
# --- ALTERAÇÃO 3: Adicionar a chave 'nome' ao criar novos trechos ---
def adicionar_item(tipo_lista):
    novo_id = time.time()
    # Adiciona a chave 'nome' com valor vazio. O nome padrão será gerado na interface.
    st.session_state[tipo_lista].append({"id": novo_id, "nome": "", "comprimento": 10.0, "diametro": 100.0, "material": "Aço Carbono (novo)", "acessorios": []})
...
def adicionar_ramal_paralelo():
    ...
    novo_id = time.time()
    # Adiciona a chave 'nome' com valor vazio.
    st.session_state.ramais_paralelos[novo_nome_ramal] = [{"id": novo_id, "nome": "", "comprimento": 50.0, "diametro": 80.0, "material": "Aço Carbono (novo)", "acessorios": []}]
...
if st.session_state.get("authentication_status"):
    ...
    with st.sidebar:
        # (O código do sidebar permanece o mesmo até a seção da Rede de Tubulação)
        ...
        with st.expander("📚 Gerenciador da Biblioteca"):
            # ... (código da biblioteca sem alterações)
        ...
        # --- ALTERAÇÃO 4: Gerar nomes padrão para trechos sem nome ---
        with st.expander("1. Trechos em Série (Antes da Divisão)"):
            for i, trecho in enumerate(st.session_state.trechos_antes):
                # Se o trecho não tiver um nome (novo ou de um cenário antigo), atribui um padrão
        ...
        with st.expander("2. Ramais em Paralelo"):
            for nome_ramal, trechos_ramal in st.session_state.ramais_paralelos.items():
                with st.container(border=True):
                    ...
                    for i, trecho in enumerate(trechos_ramal):
                        # Atribui um nome padrão para trechos dentro de ramais
                        if 'nome' not in trecho or not trecho.get('nome'):
        ...
        with st.expander("3. Trechos em Série (Depois da Junção)"):
            for i, trecho in enumerate(st.session_state.trechos_depois):
                # Atribui um nome padrão para trechos depois
                if 'nome' not in trecho or not trecho.get('nome'):
